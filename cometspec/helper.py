...
- :func:`get_meftah_irradiance_path`, :func:`open_meftah_irradiance`
    Locate and open the `Meftah et al. (2023) <https://www.mdpi.com/2072-4292/15/14/3560>`_ solar irradiance [4]_.
- :func:`get_bromley_irradiance_path`, :func:`open_bromley_irradiance`
    Locate and open the packaged Bromley solar irradiance.
- :func:`load_cn_linelist`
    Load the packaged CN line lists for a given isotopologue [6]_ [7]_.
"""
...
def make_fwhm_lambda_bounds(
    *,
    ...
    tuple[callable, callable]
        A pair ``(fwhm_min, fwhm_max)`` of callables that evaluate the minimum and maximum FWHM in arcsec as a function of wavelength given the max and min seeing and zenith angles. Each callable takes ``x : float or ndarray of float`` and returns ``f(x) : float or ndarray of float of the same shape``.
...
def throughput_vs_lambda(
    *,
    ...
    dict
        ...
        * ``lambda_nm`` (:class:`numpy.ndarray` of :class:`float`) -- Wavelength grid, in nm.
        * ``fwhm_min_arcsec`` (:class:`numpy.ndarray` of :class:`float`) -- PSF FWHM at best seeing (``eps_min``, ``zmin``)
          as a function of wavelength, in arcsec. Numerically the smallest FWHM values.
        * ``fwhm_max_arcsec`` (:class:`numpy.ndarray` of :class:`float`) -- PSF FWHM at worst seeing (``eps_max``, ``zmax``)
          as a function of wavelength, in arcsec. Numerically the largest FWHM values.
        * ``frac_min`` (:class:`numpy.ndarray` of :class:`float`) -- Enclosed flux fraction at best seeing conditions.
          Numerically the *largest* fraction (sharpest PSF → most flux within aperture).
        * ``frac_max`` (:class:`numpy.ndarray` of :class:`float`) -- Enclosed flux fraction at worst seeing conditions.
          Numerically the *smallest* fraction (broadest PSF → least flux within aperture).
        * ``loss_min`` (:class:`numpy.ndarray` of :class:`float`) -- Slit loss at best seeing, i.e. ``1 - frac_min``.
          Numerically the *smallest* loss.
        * ``loss_max`` (:class:`numpy.ndarray` of :class:`float`) -- Slit loss at worst seeing, i.e. ``1 - frac_max``.
...
def find_spectrum_file(
    ...
    *,
    ...
    fibre : str
        Fibre substring to match in the filename. Technically the function checks if both ``night`` and ``fibre`` are substrings of the filename, so they can be any distinctive part of the filename as long as they uniquely identify the file. If multiple files match, the first one when sorted by name is returned.
...
def open_hall_anderson_irradiance(wave_max_AA: float = 2990.0) -> pd.DataFrame:
    ...
    wave = air_to_vac(np.array(wave) * u.AA).value
...
def get_meftah_irradiance_path() -> Path:
    """Return the path to the packaged Meftah et al. (2023) solar irradiance file. 
    See `Meftah et al. (2023) <https://www.mdpi.com/2072-4292/15/14/3560>`_.

    Returns
    -------
    pathlib.Path
        Path to ``Meftah_MPS_ATLAS_Kurucz_LATMOS_Meftah_V1.txt``.
    ...
    FileNotFoundError
        If the packaged file is missing.
    """
    candidate = DATA_DIR / "Meftah_MPS_ATLAS_Kurucz_LATMOS_Meftah_V1.txt"
    if not candidate.exists():
        raise FileNotFoundError(
            "Meftah et al. (2023) solar irradiance file not found in data/. "
            "Place the file there or provide your own path when loading pumping spectra."
        )
    return candidate
...
def open_meftah_irradiance() -> pd.DataFrame:
    ...
    as :func:`open_kurucz_irradiance`. See `Meftah et al. (2023) <https://www.mdpi.com/2072-4292/15/14/3560>`_.
    ...
    ------
    FileNotFoundError
        If the packaged Meftah et al. (2023) file cannot be found.
    
    References
    ----------
        .. [4] Meftah, M., Sarkissian, A., Keckhut, P., & Hauchecorne, A. 2023, Remote Sens., 15, 3560. (`link <https://ui.adsabs.harvard.edu/abs/2023RemS...15.3560M>`_)
    """
    path = get_meftah_irradiance_path()
    df = pd.read_csv(path, sep=r'\s+', names=['AA', 'flux'])
    wave = np.asarray(df['AA'], dtype=float)
    wave = wave * 10.0
    flux = np.asarray(df['flux'], dtype=float)
    flux = flux * u.W / (u.m**2 * u.nm)
    flux = flux.to(u.erg / (u.s * u.cm**2 * u.AA)).value
    return pd.DataFrame({'WAVE': wave, 'FLUX': flux})


def get_bromley_irradiance_path() -> Path:
    ...
    FileNotFoundError
        If the packaged file is missing.
    """
    candidate = DATA_DIR / "Bromley.npz"
    if not candidate.exists():
        raise FileNotFoundError(
            "Bromley solar irradiance file not found in data/. "
            "Place the file there or provide your own path when loading pumping spectra."
        )
    return candidate


def open_bromley_irradiance() -> pd.DataFrame:
    r"""Load the packaged Bromley solar irradiance file.

    Bromley et. al. (2024) [5]_. file, stored as a compressed ``.npz`` archive with two
    ``float32`` arrays: ``wavelength`` (nm) and ``flux``
    (:math:`\mathrm{W\,m^{-2}\,nm^{-1}}`). The output is converted to the same
    units as :func:`open_kurucz_irradiance`.We used the 0-14000_vac.tar.gz file.
    ...
    ------
    FileNotFoundError
        If the packaged Bromley file cannot be found.
    
    References
    ----------
        .. [5] Bromley, S. J., Noonan, J. W., Cochran, A. L., et al. 2024, MNRAS, 528, 7358. (`link <https://academic.oup.com/mnras/article/528/4/7358/7607375>`_)
    """
    path = get_bromley_irradiance_path()
    with np.load(path) as z:
        wave_nm = np.asarray(z['wavelength'], dtype=float)
        flux_W_m2_nm = np.asarray(z['flux'], dtype=float)
    wave = wave_nm * 10.0
    flux = flux_W_m2_nm * u.W / (u.m**2 * u.nm)
    flux = flux.to(u.erg / (u.s * u.cm**2 * u.AA)).value
    return pd.DataFrame({'WAVE': wave, 'FLUX': flux})
...
def load_cn_linelist(path_or_text: Union[str, os.PathLike]) -> pd.DataFrame:
    ...
    table" format used by ApJS, as produced by Brooke et al. (2014) [6]_ and
    Sneden et al. (2014) [7]_. Three isotopologues are supported, each available
    ...
    pandas.DataFrame
        ...
        * ``eS'`` (:class:`str`) -- Upper electronic state label (``A``, ``B``, or ``X``).
        * ``eS''`` (:class:`str`) -- Lower electronic state label (``A``, ``B``, or ``X``).
        * ``v'`` (:class:`int`) -- Upper vibrational quantum number :math:`v'`.
        * ``v''`` (:class:`int`) -- Lower vibrational quantum number :math:`v''`.

        Rotational quantum numbers and fine-structure / parity labels

        * ``J'`` (:class:`float`) -- Upper total angular momentum :math:`J'` (excluding nuclear spin).
        * ``J''`` (:class:`float`) -- Lower total angular momentum :math:`J''`.
        * ``F'`` (:class:`int`) -- Upper-state spin/parity component: in :math:`A^{2}\Pi`, ``1`` for :math:`\Omega = 1/2`, ``2`` for :math:`\Omega = 3/2`; in :math:`B^{2}\Sigma^{+}` and :math:`X^{2}\Sigma^{+}`, ``1`` for :math:`e`, ``2`` for :math:`f` parity.
        * ``F''`` (:class:`int`) -- Lower-state spin/parity component, same encoding as ``F'``.
        * ``p'`` (:class:`str`) -- Upper-state parity / e-f label (``e`` or ``f``).
        * ``p''`` (:class:`str`) -- Lower-state parity / e-f label (``e`` or ``f``).
        * ``N'`` (:class:`float`) -- Upper :math:`N` quantum number as defined in Brooke et al. (2014) [6]_. Stored as text in the source file (allowing blank entries) and coerced to numeric; ``NaN`` where blank.
        * ``N''`` (:class:`int`) -- Lower :math:`N` quantum number.
        * ``Obs`` (:class:`float`) -- Observed transition wavenumber, in cm\ :sup:`-1`. May be  ``NaN`` for lines without a measured position (predicted-only).
        * ``Cal`` (:class:`float`) -- Calculated transition wavenumber from the term-value fit, in cm\ :sup:`-1`.
        * ``Res`` (:class:`float`) -- Residual ``Obs - Cal``, in cm\ :sup:`-1`.
        * ``E''`` (:class:`float`) -- Lower-state energy :math:`E''` relative to ``v"=0``, in cm\ :sup:`-1`.
        * ``A`` (:class:`float`) -- Einstein :math:`A` coefficient (spontaneous emission rate), in s\ :sup:`-1`.
        * ``f`` (:class:`float`) -- Absorption oscillator strength (dimensionless).
        * ``Des`` (:class:`str`) -- Transition description from Brooke/Sneden line list [6]_ [7]_.
        * ``lambda_vac_A_from_Cal`` (:class:`float`) -- Vacuum wavelength in Å, computed as :math:`10^{8}/\mathrm{Cal}`.
        * ``lambda_vac_A_from_Obs`` (:class:`float`) -- Vacuum wavelength in Å, computed as :math:`10^{8}/\mathrm{Obs}`. ``NaN`` where ``Obs`` is missing.
    ...
    References
    ----------
    .. [6] Brooke, J. S. A., Ram, R. S., Western, C. M., et al. 2014, ApJS, 210, 23 (`link <https://doi.org/10.1088/0067-0049/210/2/23>`__).
    .. [7] Sneden, C., Lucatello, S., Ram, R. S., Brooke, J. S. A., & Bernath, P. 2014, ApJS, 214, 26 (`link <https://doi.org/10.1088/0067-0049/214/2/26>`__).
    """
